...
/**
 * The fully self-contained Document Helper tool: category picker, dynamic
 * field form, and generated-text result panel. Used both on the standalone
 * `/tools/document-helper` page and inline on the homepage.
 *
 * The result panel carries `data-print-area="true"` so the "Print" button's
 * `window.print()` call only prints this panel (see the `@media print` rule
 * in `index.css`) — this works correctly even when several tools are
 * mounted at once (the homepage), since it targets this element directly
 * rather than relying on a page-unique id.
 */
export default function DocumentHelperTool() {
